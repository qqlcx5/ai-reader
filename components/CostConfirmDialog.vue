  ...
  >
    <div class="bg-white dark:bg-gray-900 rounded-lg shadow-xl w-[90vw] max-w-md overflow-hidden">
      <div class="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-700">
        ...
        <button @click="emit('cancel')" class="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors" aria-label="关闭">
          <X class="w-4 h-4" />
        </button>
      </div>
      <div class="px-4 py-3">
        ...
        <div class="space-y-2">
          <div v-for="m in models" :key="m.name" class="flex items-center justify-between text-sm py-1.5 px-3 bg-gray-50 dark:bg-gray-800 rounded">
      ...
      <div class="px-4 py-3 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between">
        <label class="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 cursor-pointer">
          <input v-model="dontAskAgain" type="checkbox" class="rounded border-gray-300 dark:border-gray-600" />
          本次不再提示
        </label>
        <div class="flex items-center gap-2">
          <button @click="emit('cancel')" class="px-3 py-1.5 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 transition-colors">取消</button>
          <button @click="handleConfirm" class="flex items-center gap-1.5 px-4 py-1.5 text-sm bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors">
            <Send class="w-3.5 h-3.5" /> 确认发送
          </button>
        </div>
      </div>
    </div>
