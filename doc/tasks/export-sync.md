...
- [ ] 安装 `jszip`
- [ ] 实现 `lib/export/zip.worker.ts`（Web Worker）
...
- [ ] 实现主线程 `lib/export/zip.ts`（Worker 封装）
  - `exportAll(): Promise<void>`：启动 Worker，监听进度
- [ ] 实现 `components/export/ExportProgressModal.vue`
...
- [ ] 实现 `components/export/ImportPanel.vue`
...
- [ ] 实现 `lib/export/scheduler.ts`（WebDAV 定时同步）
...
- [ ] 实现 `components/export/SyncOptionsPanel.vue`
...
- [ ] 自动生成 YAML Frontmatter：
  ...
  ---
  ```
- [ ] Options 设置：Obsidian Vault 名称输入框 + 默认目标路径（如 `AI-Reader/`）

---

## 5. 本地文件导出（单条记录）

- [ ] 实现 `lib/export/local-file.ts`
...
- [ ] 实现【复制到剪贴板】：`navigator.clipboard.writeText(markdown)`
...
- [ ] 安装 `lz-string`
- [ ] 实现 `lib/export/settings-sync.ts`
