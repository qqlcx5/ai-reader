...
- [ ] 排序：默认按 `updatedAt` 倒序；支持切换按域名分类
- [ ] 顶部搜索框：输入后触发 Web Worker 检索（见 §3）

---

## 2. 快照查看器（主副表懒加载）

- [ ] 实现 `components/library/SnapshotViewer.vue`
...
- [ ] 实现 `lib/library/search.ts`（主线程接口）
  - `search(query: string): Promise<SearchResult[]>`（封装 Worker 通信）
  - debounce 300ms 防抖，避免输入时高频触发
- [ ] 搜索结果高亮渲染：命中关键词背景色 `#fff299`，文字 `#5c4a00`
- [ ] 无结果时展示 `EmptyState`（"未找到相关记录"）

---

## 4. 搜索结果列表

- [ ] 实现 `components/library/SearchResultList.vue`
...
- [ ] 删除单条记录：长按 / 右键 → 确认 Modal → 同时删除主表 + 副表 + 关联 Highlights
- [ ] 批量删除：勾选模式（Checkbox），批量确认后事务性删除
- [ ] 按域名筛选：左侧域名导航列表（类书签分组），点击过滤 `HistoryList`
...
- [ ] 在 M1 右栏"历史"Tab 中嵌入 `HistoryList.vue`（紧凑模式，150px 高度，可展开）
- [ ] 在 Options 独立管理页中嵌入完整 `LibraryPage.vue`（左侧列表 + 右侧查看器分屏）
