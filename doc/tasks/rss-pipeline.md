...
- [x] 实现 `lib/rss/dedup.ts`
  - `computeArticleId(article: { link: string }): Promise<string>`：`SHA-256(article.link)` → hex（与 M8 `RSSArticleRecord.id` 一致）
  - `filterNewArticles(fetched: RawArticle[], existing: Set<string>): Promise<RawArticle[]>`
...
- [x] 实现 `lib/rss/summarizer.ts`
  - `summarizeArticle(article: RawArticle, engine: IEngine, apiKey: string, model: string): Promise<string>`
    - 构建 Prompt：`"请用 3 句话（不超过 200 字）概括以下文章内容：\n\n${article.description}"`
    - 调用 M3 `engine.chatStream()`，收集完整响应后返回摘要文本
    - 超时 30s（`AbortSignal.timeout(30000)`），失败时返回 `article.description.slice(0, 200)` 降级摘要
...
- [x] 实现 `components/rss/RSSPanel.vue`（接入 M1 右栏"RSS"Tab）
- [x] 实现 `components/rss/FeedList.vue`
  - 已订阅 Feed 列表：Feed 标题 + favicon + 未读数 badge
  - 点击展开该 Feed 的文章列表
  - 右键 / 操作按钮：删除 Feed / 立即刷新
- [x] 实现 `components/rss/ArticleItem.vue`
...
- [x] 实现 `components/rss/AddFeedModal.vue`
...
- [x] 实现 `lib/rss/daily-briefing.ts`
...
- [x] 实现 `components/rss/DailyBriefingCard.vue`
...
- [x] 在 Options 页 RSS Tab 中嵌入订阅源管理：
