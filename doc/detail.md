...
每篇文章应该有独立的对话会话列表
切换到文章A时，显示文章A关联的所有会话（会话1、会话2...）
可以新建会话
从历史库切换到某篇文章时，能加载该文章的所有历史会话
这是一个会话管理功能，涉及：

ConversationRepository 需要按 documentId 查询会话
Chat store 需要支持会话列表、切换会话、新建会话
Library View 点击文档后，Chat View 应该加载该文档的会话列表
UI 层：会话列表组件（显示会话标题、时间等）
这需要派发给 file-agent 来实现。参考 obsidian-clipper。
