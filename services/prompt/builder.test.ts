...
describe('PromptBuilder', () => {
  ...
  it('should inject context as a system message before history and user input', () => {
    const result = builder.build({
    ...
    expect(result.messages[0]).toEqual({ role: 'system', content: '# Page Content' })
  ...
  it('should handle empty history', () => {
    ...
    expect(result.messages[0]).toEqual({ role: 'system', content: 'Ctx' })
  ...
  it('should combine all fields together', () => {
    ...
    expect(result.messages[0]).toEqual({ role: 'system', content: 'Context info' })
